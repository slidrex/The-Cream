...
{
    internal class DinaAttackBehaviour : EntityBrain<Dina>
    {
        ...
        private void PerformAttack()
        {
            ...
            if (target != null && target is IDamageable && _data.IsReachedTarget)
            {
                ...
                {
                    damageable.Damage((int)_attackComponent.GetValue() + 3, Entity);
                    var bite = Instantiate(biteEffect, target.transform.position, Quaternion.identity);
                    Destroy(bite, 0.45f);
                }
            }
        }
